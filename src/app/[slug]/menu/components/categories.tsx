...
const RestaurantCategories = ({restaurant}: RestaurantCategoriesProps) => {
    ...
    const {products, total, toogleCart, totalQuantity} = useContext(CartContext);
        ...
        <div className={"relative z-50 mt-[-1.5rem] rounded-t-3xl bg-white"}>
            ...
            <ScrollArea className={"w-full "}>
                <div className={"flex w-max space-x-4 p-4 pt-0"}>
                    {restaurant.menuCategories.map(category => (
                        <Button key={category.id} variant={getCategoryButtonVariant(category)} size={"sm"}
                                className={"rounded-full"} onClick={() => handleCategoryClick(category)}>
            ...
            {products.length > 0 && (
                <div className={"fixed bottom-0 left-0 right-0 flex w-full items-center justify-between border border-t bg-white px-5 py-3"}>
                    ...
                    <Button onClick={toogleCart}>Ver Sacola</Button>
                    <CartSheet/>
                </div>
                )}
        </div>
